...
const restoreUSB = (driveVolume, driveLetter) => {
...
`;
  const scriptPath = path.join(__dirname, `restoreDriveDiskpartScript_${Date.now()}_${driveLetter}.txt`);
  ...
  exec(`diskpart /s "${scriptPath}"`, (error, stdout, stderr) => {
    ...
    fs.unlink(scriptPath, (err) => {if (err) console.error(err)})
  });
}
